...
class listEstudiantesView(APIView):
    ...
    def get_queryset(self):
        ...
        rut_al = self.request.query_params.get('rut_al', None)
        uid = self.request.query_params.get('uid', None)

        if rut_al is not None:
            queryset = queryset.filter(rut_al=rut_al)
...
class EstudiantePorUidView(APIView):
    permission_classes = (permissions.AllowAny,)
    serializer_class = EstudianteSerializer

    def get_queryset(self):
        rut_al = self.kwargs['rut_al']
        return DjangoEstudiante.objects.filter(rut_al=rut_al)

    def get(self, request, rut_al, format=None):
        estudiantes = self.get_queryset()
        ...
        if estudiantes.exists():
            ...
            return Response({'error': 'No hay estudiantes con ese uid'}, status=status.HTTP_404_NOT_FOUND)